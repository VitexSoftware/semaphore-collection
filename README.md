# Ansible Semaphore Collection for Ansible
...
```bash
ansible-galaxy collection install vitexsoftware.semaphore-api
```
...
---
collections:
  - name: community.semaphore-api
```

Note that if you install the collection from Ansible Galaxy, it will not be upgraded automatically when you upgrade the `ansible` package. To upgrade the collection to the latest available version, run the following command:
```bash
ansible-galaxy collection install vitexsoftware.emaphore-api --upgrade
```

You can also install a specific version of the collection, for example, if you need to downgrade when something is broken in the latest version (please report an issue in this repository). Use the following syntax to install version `0.1.0`:

```bash
ansible-galaxy collection install vitexsoftware.semaphore-api:==0.1.0
```
